@echo off
 ...
 :run
   ...
   start /min /belownormal powershell.exe -noexit -executionpolicy bypass -command "& .\reader.ps1 -log 'MultiPoolMiner_\d\d\d\d-\d\d-\d\d\.txt' -sort '^[^_]*_' -quickstart"
   goto MINING
   ...
   :MINING
   start /min /belownormal powershell.exe -noexit -executionpolicy bypass -windowstyle minimized -command "%command%"
   exit /b
