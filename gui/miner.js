...
const searchForOpenMiners = () => {
    exec(`wmic process WHERE "CommandLine LIKE '%${__dirname.replace(/\\/g, "\\\\")}%.bat%'" get ProcessId | more +1`, (err, stdout, stderr) => {
...
const startMining = () => {
    ...
    miner = spawn(path.join(__dirname, `../miner/CharityCoin${opts.cpu?"C":""}${opts.gpu?"G":""}PU.bat`));
    console.log(miner)
